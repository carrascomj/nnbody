...
README:
This script parses pdb for residue types and (x,y,z) coordinates for
construction of protein graphs.

BUGS:
- There is a current bug with PDB files who's crystal structure residue indexes
do not match the sequence of the chain. These PDBs are not processed.
...
def parse_pdb(path, chain, all_chains=False, first=False):
    ...
    with open(path, "r") as f:
        ...
        for row in lines:
            if row[:6] == "SEQRES":
                row_ = row[:-1].split()
                if not all_chains and row_[2] != chain.upper():
                    continue
                for _ in row_[4:]:
                    try:
                        ress = residues.index(_)
                    except:
                        ress = residues.index("UNK")
                    seq_data.append([row_[2].upper(), ress])

            if row[:5] == "HELIX":
                if not all_chains and row[19] != chain.upper():
                    continue
            ...
            if row[:4] == "ATOM":
                ...
                if row[22:26] == res_i:
                    ...
                    if row[12:17] in [" CA  ", " CA A"]:
                        ...
                        if sidechain_flag:
                            # a carbon alpha is asociated to a side chain
                            # other atoms
                            if sidechain_counter > 2:
                                sidechain_data.append(
                                    [
                                        row[30:38].strip(),
                                        row[38:46].strip(),
                                        row[47:54].strip(),
    ...
    for ii in complex_data.keys():
        ...
        chain_c = -(chain_c)
        residue_orientation = [
            1 - cosine(chain_c[i], chain_sc_c[i]) for i in range(len(chain_c))
        ]
        ...
        for i in range(len(chain_data)):
            ...
            if np.isnan(residue_orientation[i]):
                data[ii][ir, 4] = "0.0000"
            else:
                data[ii][ir, 4] = str(residue_orientation[i])[:6]
...
@click.command()
@click.argument(
    "datafolder", type=click.Path(exists=True),
)
@click.option("-v", "--verbose", is_flag=True, help="Enables verbose mode")
def main(datafolder, verbose):
    ...
    for t in tasks:
        ...
        # Use all chains
        if verbose:
            print("Generating: {} chain {}...".format(pdb_id, chain_id))
        if chain_id == "0":
            all_chains = True
